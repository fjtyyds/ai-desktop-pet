...
- 当前任务：M2 全部功能已合并（T-05/T-06/T-07/T-08），等待 `npm run dev` 人工目检
- 最近完成：M0~M1 全部验收通过；真实 DeepSeek 调用验证；M2 设计；T-05~T-08 开发与集成；ADR-013/014
- 下一步：`npm run dev` 人工目检 M2（重启历史恢复、人格/情绪设置、记忆引用）；真实 API Key 下验证异步记忆抽取
- 阻塞：无
