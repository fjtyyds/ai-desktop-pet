...
- 更新时间：2026-08-10
- 当前阶段：M3.5 收尾优化全部完成（T-24~T-31 + T-33）；用户目检两处问题修复中：TTS（T-34 已合并待试听）、贴边（T-35 待派发）
- 当前任务：T-34 已合并（6b343db），待用户试听；T-35 贴边吸附修复（派发中）
- 最近完成：T-34 Edge 在线神经语音完成并合并（worker check/smoke + 协调者双音色合成验证通过）；ADR-028/029 落盘
- 下一步：派发并验收 T-35 → 用户复核 TTS 试听与贴边；随后 M4 分发规划与 UI 大改（M3.6）评估
- 阻塞：无
