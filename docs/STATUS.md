...
- 更新时间：2026-08-10
- 当前阶段：M3.5 收尾优化全部完成；T-34（神经语音）与 T-35（贴边吸附）已合并；TTS 听感优化（T-36）与人工复核进行中
- 当前任务：T-36 TTS 听感优化（96kbps 输出 + 语调参数重调，待派发）；用户复核贴边（真人拖动）
- 最近完成：T-34（6b343db）、T-35（98d1de4）验收合并；免费端点能力实测（express-as/break 不可用，96kbps 可用）
- 下一步：派发 T-36 → 用户复核贴边与 v2 听感 → M4 分发规划与 UI 大改（M3.6）评估
- 阻塞：无
