...
## ADR-047：总工交接必须保持项目上下文（2026-08-13）

- 状态：Accepted（事故复盘后固化）
- 背景：2026-08-13 22:34 总工交接再次把新总工创建到项目外——“codex-context-handoff”技能给出的交接命令未带 `--project`，`send-prompt.mjs` 无项目参数时走“新对话/不在项目中工作”路径，生成项目外（projectless）线程。项目外线程读不到本项目 AGENTS/上下文，无法履行项目总工职责，并造成双总工风险（此前已至少发生一次，用户明确要求强化重视）。
- 决策：
  1. 项目总工交接命令必须为 `send-prompt.mjs --file <交接提示词> --project "E:\codex\AI桌宠"`；`--project` 为项目总工交接的必填参数，禁止省略或回退到“不在项目中工作”。
  2. 交接文件新增“项目根目录”字段；新总工上岗前必须确认本线程属于该项目（send-prompt 输出出现“在 <项目名> 中开始新聊天”，或截图/线程树可见项目上下文）。
...
## ADR-055：桌面快捷方式随最新版同步自动替换（2026-08-14）

- 状态：Accepted（用户明确要求“每次改动后都把桌面的AI桌宠的快捷方式替换成最新的版本”，
  2026-08-14 实施）
- 背景：此前桌面没有 AI桌宠 快捷方式；用户希望每次改动后桌面快捷方式都指向最新版本，
  避免误用旧构建。
- 决策：
  1. 规则写入 AGENTS.md 验收要求：每次 main 合并验收并执行 sync-latest 后，
     自动把桌面快捷方式替换为最新可运行版本（锚定在既有 ADR-042 同步节奏上，
     不在每次局部编辑时触发）。
  2. `scripts/sync-latest.ps1` 末尾新增“桌面快捷方式”步骤：创建/替换
     `%USERPROFILE%\Desktop\AI桌宠.lnk`，Target 指向本次构建的
     `E:\codex\AI桌宠\dist\win-unpacked\AI桌宠.exe`（与安装包同源的最新可运行产物），
     工作目录与图标取该 exe；脚本保持纯 ASCII（文件名用 Unicode 码点构造）。
  3. win-unpacked 产物或 exe 缺失时不覆盖旧快捷方式并告警，不中断同步。
- 后果：桌面双击即可启动最新构建；安装包/自动更新链路（ADR-042/043）不变；
  sync-latest 成为“构建→同步→快捷方式”的完整收尾步骤。
