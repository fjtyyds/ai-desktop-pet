...
背景：2026-08-10 整体目检后用户反馈 9 类问题，详见 `docs/reports/2026-08-10-M3.5-整体目检与优化方案.md` 与 ADR-025。

建议任务（确认后立卡，一个会话一个任务）：

- T-24 窗口布局：可缩放、小部件不遮挡聊天、移除底部平台/版本信息
- T-25 工具栏：导出移入工具栏 + 新增最小化按钮
- T-26 天气：缩短自动刷新间隔（建议 15 分钟）+ 手动刷新/更新时间提示优化
- T-27 番茄钟：结束通知重复弹窗修复（幂等消费 + 清零竞态）
- T-28 内容精简：人格模板文案精简；全局快捷键移除（待确认）；系统状态小部件移除（待确认）
- T-29 贴边体验：先确认预期行为（自动隐藏 vs 靠边吸附）再实现
